...
import yc.pointer.trip.event.NewItemHomeEvent;
 ...
 * 描述:新版首页0809
 */
public class NewItemHomeFragment extends BaseViewPageFragment implements NewHomeFragmentMoveItemAdapter.ItemHomeMoveRecycler,
    ...
    public static Fragment newInstance(int key) {
        NewItemHomeFragment fragment = new NewItemHomeFragment();
    ...
    protected void initView() {
        ...
        if (!OkHttpUtils.isNetworkAvailable(getActivity())) {
            mItemOne = MySQLiteDataBase.findAll();
            if (mItemOne.size() > 0) {
    ...
    private void requestHomePageData(String userId, String devcode, Long timestamp, int page, int type) {
        ...
        if (timeFlag) {
            ...
            RequestBody requestBody = new FormBody.Builder()
                    ...
                    .add("uid", userId)
                    .build();
            OkHttpUtils.getInstance().post(URLUtils.HOME_VIDEO_NEW_DATA, requestBody, new HttpCallBack(new NewItemHomeEvent()));
        }
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void requestHomePageData(NewItemHomeEvent event) {
        if (event.isTimeOut()) {
...
//        mLikeNumText.setEnabled(false);
    ...
    private void postFollow(String userId, String devcode, long timestamp, String uuid, int status) {
        if (APPUtils.judgeTimeDev(getActivity(), devcode, timestamp)) {
            String sign = Md5Utils.createMD5("devcode=" + devcode + "status=" + status + "timestamp=" + timestamp + "uid="
                    + userId + "uuid=" + uuid + URLUtils.WK_APP_KEY);
     ...
     * @param bookBean
     */
    private ContentValues setBookValues(ContentValues values, BookBean bookBean) {
        values.put("bid", bookBean.getBid());
        values.put("uid", bookBean.getUid());
        values.put("title", bookBean.getTitle());
        values.put("type", bookBean.getType());
        values.put("city", bookBean.getCity());
        values.put("spot", bookBean.getSpot());
        values.put("b_pic", bookBean.getB_pic());
        values.put("y_pic", bookBean.getY_pic());
        values.put("is_order", bookBean.getIs_order());
        values.put("is_ad", bookBean.getIs_ad());
        values.put("is_index", bookBean.getIs_index());
        values.put("look_num", bookBean.getLook_num());
        values.put("zan_num", bookBean.getZan_num());
        values.put("ord_num", bookBean.getOrd_num());
        values.put("status", bookBean.getStatusX());
        values.put("info", bookBean.getInfo());
        values.put("is_cao", bookBean.getIs_cao());
        values.put("nickname", bookBean.getNickname());
        values.put("phone", bookBean.getPhone());
        values.put("pic", bookBean.getPic());
        values.put("cp", bookBean.getCp());
        values.put("video", bookBean.getVideo());
        values.put("col_num", bookBean.getCol_num());
        values.put("width", bookBean.getWidth());
        values.put("height", bookBean.getHeight());
        values.put("is_video", bookBean.getIs_video());
        values.put("location", bookBean.getLocation());
        values.put("addtime", bookBean.getAddtime());
        values.put("addtime1", bookBean.getAddtime1());
        return values;
    }
     ...
     * @return
     */
    private List<BookBean> selectAllData(String sql, SQLiteDatabase database) {
        List<BookBean> list = new ArrayList<>();
        Cursor cursor = database.rawQuery(sql, null);
        if (cursor != null) {
            while (cursor.moveToNext()) {
                BookBean bookBean = selectColumn(cursor);
                list.add(bookBean);
            }
            cursor.close();
        }
    ...
    private BookBean selectColumn(Cursor cursor) {
        ...
        String b_pic = cursor.getString(cursor.getColumnIndex("b_pic"));
        String y_pic = cursor.getString(cursor.getColumnIndex("y_pic"));
        String is_order = cursor.getString(cursor.getColumnIndex("is_order"));
        String is_ad = cursor.getString(cursor.getColumnIndex("is_ad"));
        String is_index = cursor.getString(cursor.getColumnIndex("is_index"));
        String look_num = cursor.getString(cursor.getColumnIndex("look_num"));
        String zan_num = cursor.getString(cursor.getColumnIndex("zan_num"));
        String ord_num = cursor.getString(cursor.getColumnIndex("ord_num"));
        String status = cursor.getString(cursor.getColumnIndex("status"));
        String info = cursor.getString(cursor.getColumnIndex("info"));
        String is_cao = cursor.getString(cursor.getColumnIndex("is_cao"));
        ...
        String col_num = cursor.getString(cursor.getColumnIndex("col_num"));
        String width = cursor.getString(cursor.getColumnIndex("width"));
        String height = cursor.getString(cursor.getColumnIndex("height"));
        String is_video = cursor.getString(cursor.getColumnIndex("is_video"));
        String location = cursor.getString(cursor.getColumnIndex("location"));
        String addtime = cursor.getString(cursor.getColumnIndex("addtime"));
        String addtime1 = cursor.getString(cursor.getColumnIndex("addtime1"));
        bookBean.setBid(bid);
        bookBean.setUid(uid);
        bookBean.setTitle(title);
        bookBean.setType(type);
        bookBean.setCity(city);
        bookBean.setSpot(spot);
        bookBean.setB_pic(b_pic);
        bookBean.setY_pic(y_pic);
        bookBean.setIs_order(is_order);
        bookBean.setIs_ad(is_ad);
        bookBean.setIs_index(is_index);
        bookBean.setLook_num(look_num);
        bookBean.setZan_num(zan_num);
        bookBean.setOrd_num(ord_num);
        bookBean.setStatusX(status);
        bookBean.setInfo(info);
        bookBean.setIs_cao(is_cao);
        bookBean.setNickname(nickname);
        bookBean.setPhone(phone);
        bookBean.setPic(pic);
        bookBean.setCp(cp);
        bookBean.setVideo(video);
        bookBean.setCol_num(col_num);
        bookBean.setWidth(width);
        bookBean.setHeight(height);
        bookBean.setIs_order(is_video);
        bookBean.setLocation(location);
        bookBean.setAddtime(addtime);
        bookBean.setAddtime1(addtime1);
        return bookBean;
    }

...
public class MovieRecorderView extends LinearLayout implements MediaRecorder.OnErrorListener {
    ...
    public MovieRecorderView(Context context, AttributeSet attrs, int defStyle) {
        ...
        recordMaxTime = a.getInteger(R.styleable.MovieRecorderView_record_max_time, 30);//默认最大拍摄时间为10s
     ...
     * @throws IOException
     */
    public void initCamera(int type) throws IOException {
        if (camera != null) {
            freeCameraResource();
        }
        try {
            if (hasCamera()) {//检查是否又摄像头
                if (type == 1) {
                    camera = Camera.open(Camera.CameraInfo.CAMERA_FACING_BACK);//TODO 默认打开后置摄像头
                } else if (type==0){
                    camera = Camera.open(Camera.CameraInfo.CAMERA_FACING_FRONT);//TODO 默认打开后置摄像头
                }
            }
            //checkCameraFacing(camera,type);
            parameters = camera.getParameters();
            //toggleFlashMode();//设置闪光灯
            //设置自动连续对焦
...
//        params.setPictureSize(480, 720);//拍照保存比率
    ...
    private void createRecordDir() {
        File sampleDir = new File(Environment.getExternalStorageDirectory() + File.separator + "trip/video/");
        if (!sampleDir.exists()) {
            sampleDir.mkdirs();
        }
        try {
     ...
     * 录制视频初始化
     */
    private void initRecord(int type) throws Exception {
...
//        if (sizePicture < 3000000) {//这里设置可以调整清晰度
        ...
        if (type == 1) {
            ...
            //横屏
            mediaRecorder.setOrientationHint(0);//输出旋转90度，保持竖屏录制
        }
     ...
     */
    public void record(final OnRecordFinishListener onRecordFinishListener, int type) {
    ...
    public boolean toggleFlashMode() {
        if (parameters != null) {
            try {
                ...
                if (!StringUtil.isEmpty(mode) || Camera.Parameters.FLASH_MODE_OFF.equals(mode))
